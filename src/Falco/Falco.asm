...
scope Falco {
    ...
    Character.edit_action(FALCO, 0xE2,              -1,             0x800D94E8,                 Phantasm.air_subroutine_,       Phantasm.air_physics_,          Phantasm.air_collision_)
    ...
    scope up_special_velocity_1_: {
        ...
        ori     t1, r0, Character.id.FALCO  // t1 = FALCO
        beq     t0, t1, _end                // branch if character = FALCO
        lui     at, 0x42cc                  // up special velocity = 0x42C40000

        lui     at, 0x4315                  // up special velocity = 0x42E60000 (original line 1)
        _end:
        lw      t0, 0x0004(sp)              // ~
        lw      t1, 0x0008(sp)              // load t0, t1
        addiu   sp, sp, 0x0010              // deallocate stack space
